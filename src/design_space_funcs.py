...
def map_von_mises_stresses_to_grid(vm_stresses, grid):
    ...
    von_mises_grid = np.zeros((rows, cols))
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            if grid[0, i, j] == 0.0:
                ...
                temp[i, j] = (vm_stresses[element_id] + vm_stresses[element_id_2]) / 2
            else:
                von_mises_grid[i, j] = (vm_stresses[element_id] + vm_stresses[element_id_2]) / 2
                if von_mises_grid[i, j] > max_value:
                    max_value = von_mises_grid[i, j]
                if von_mises_grid[i, j] < min_value:
                    min_value = von_mises_grid[i, j]

    von_mises_grid = max_value + 1 - von_mises_grid
    von_mises_grid[grid[0,:,:] == 0] = 0
    von_mises_grid[grid[1, :, :] == 1] = 0
    max_vm = np.max(von_mises_grid.flatten())
    von_mises_grid = von_mises_grid / max_vm
    von_mises_grid = von_mises_grid + temp
    von_mises_grid[grid[0,:,:] == 0] = 0

...
###############################
    ...
    node_von_mises_stresses = np.zeros(num_nodes)
    node_von_mises_count = np.zeros(num_nodes)
    ...
    for c in conn:  # for each element (conn is Nx4)
        ...
        for q in q4:  # for each integration pt, eg: [-0.7,-0.7]
            ...
            if not is_voided:
                for strain_comp in strain:
                    total_sum_strain += strain_comp
            ...
            if not is_voided:
                for node in c[:4]:
                    node_von_mises_stresses[node] += von_mises_stress
                    node_von_mises_count[node] += 1
    ...
    node_von_mises_stresses = node_von_mises_stresses / np.max(node_von_mises_stresses)
    ...
    if plot_flag:
        ...
        ##############################
        # print('\n** Plot displacement')
        global file_counter, current_subdirectory
        any_voided = any(c[4] for c in conn)

        grid_dimensions = grid[0,:,:].shape
        current_subdirectory = "Undef"

    # If no element is voided, reset the file name counter and create a new subdirectory
        if not any_voided:
            file_counter = 0
            current_time = datetime.datetime.now()
            subdirectory_name = f"{grid_dimensions[0]}x{grid_dimensions[1]}_{current_time.strftime('%Y-%m-%d_%H-%M')}"
            current_subdirectory = subdirectory_name
            save_dir = os.path.join(const.FIG_DIR, current_subdirectory)
            os.makedirs(save_dir, exist_ok=True)
        else:
            if current_subdirectory is None:
                save_dir = os.path.join(const.FIG_DIR, "results")
            save_dir = os.path.join(const.FIG_DIR, current_subdirectory)
        ...
        for ni, pt in enumerate(nodes):
            ...
            if plot_type == 'von_mises': res.append(node_von_mises_stresses[ni])
        ...
        #äplt.scatter(xvec, yvec, marker='o', c='b', s=0.5) # (plot the nodes)
        plt.grid(False)  # Turn off the grid
        plt.xticks([])  # Turn off the x-axis labels
        plt.yticks([])  # Turn off the y-axis labels
        plt.colorbar(t)
        plt.title("Relative von Mises Stress")
        plt.axis('equal')
        file_name = f"result_{file_counter}.png"
    ...
    strain_energy = (u.T @ K @ u).item()
    print(strain_energy)
    print(max_strain)
    print(max_stress)
    print(avg_strain_over_nodes)
    return von_mises_stresses, strain_energy
